...
class PatchEmbed(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        print(f"PatchEmbed {self.in_channels}, {self.temporal_patch_size}, {self.patch_size}")
...
class Qwen2VLVisionBlock(nn.Module):
    ...
    def forward(self, hidden_states, cu_seqlens, rotary_pos_emb) -> torch.Tensor:
        global prev_time, between_vision_block, vision_block
        between_vision_block += time.time() - prev_time
        prev_time = time.time()
        hidden_states = hidden_states + self.attn(
            self.norm1(hidden_states), cu_seqlens=cu_seqlens, rotary_pos_emb=rotary_pos_emb
        )
        hidden_states = hidden_states + self.mlp(self.norm2(hidden_states))
        vision_block += time.time() - prev_time
        prev_time = time.time()
        return hidden_states
        ...
        **kwargs,
        ...
        Args:
            ...
            kwargs (`dict`, *optional*):
                ...
                into the model
        """
        global prev_time, DL_premodel, DL_self_attn, DL_fc

        residual = hidden_states

        hidden_states = self.input_layernorm(hidden_states)

        DL_premodel += time.time() - prev_time
        prev_time = time.time()
        # Self Attention
        ...
        DL_self_attn += time.time() - prev_time
        prev_time = time.time()
...
class Qwen2VLModel(Qwen2VLPreTrainedModel):
    ...
    def forward(
        ...
        global prev_time, Q2_prelayers, Q2_postlayers
        ...
        Q2_prelayers += time.time() - prev_time
        prev_time = time.time()
        ...
        Q2_postlayers += time.time() - prev_time
        prev_time = time.time()
...
class Qwen2VLForConditionalGeneration(Qwen2VLPreTrainedModel):
    ...
    def forward(
        ...
        ```"""
        global prev_time, CG_prevision, CG_vision, CG_postvision, CG_postmodel
        prev_time = time.time()
        output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
        output_hidden_states = (
            output_hidden_states if output_hidden_states is not None else self.config.output_hidden_states
        ...
        CG_postvision += time.time() - prev_time
        prev_time = time.time()
        outputs = self.model(
            input_ids=None,
            position_ids=position_ids,
        ...
        CG_postmodel += time.time() - prev_time
        prev_time = time.time()
    ...
    def print_time(self):
        print(f"CG_postmodel {CG_postmodel}")
        print(f"Q2_prelayers {Q2_prelayers}")
        print(f"Q2_postlayers {Q2_postlayers}")
        print(f"DL_premodel {DL_premodel}")
        print(f"DL_self_attn {DL_self_attn}")
        print(f"DL_fc {DL_fc}")
        print(f"between_vision_block {between_vision_block}")
        print(f"vision_block {vision_block}")
        print(f"CG_prevision {CG_prevision}")
        print(f"CG_vision {CG_vision}")
        print(f"CG_postvision {CG_postvision}")
