...
from DataDAM.utils import get_loops, get_dataset, get_network, get_eval_pool, evaluate_synset, get_daparam, get_time, TensorDataset, epoch, DiffAugment, ParamDiffAug, get_attention
...
def main(dataset_func, network_func, args):
    ...
    channel, im_size, num_classes, class_names, mean, std, dst_train, dst_test, testloader = dataset_func(args.dataset, args.data_path)
    ...
    for exp in range(args.num_exp):
        ...
        for it in range(args.Iteration+1):
            ...
            if it in eval_it_pool:
                for model_eval in model_eval_pool:
                    ...
                    for it_eval in range(args.num_eval):
                        net_eval = network_func(model_eval, channel, num_classes, im_size).to(args.device) # get a random model
            ...
            net = network_func(args.model, channel, num_classes, im_size).to(args.device) # get a random model
            net.train()
            ...
            output_loss =  100*args.task_balance * error(output_real, output_syn, err_type="MSE").cpu()
